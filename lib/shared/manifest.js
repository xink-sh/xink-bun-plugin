...
export const createManifest = async (config, dev, outdir) => {
  ...
  const initMiddleware = async () => {
    ...
    for await (const file of middleware_glob.scan(middleware_dir)) {
      const src_middleware_path = `${cwd}/${middleware_dir}/${file}`
  ...
  for await (const file of routes_glob.scan(routes_dir)) {
    ...
    const dirs = parts.filter(t => t !== 'route.ts')
    ...
    const build_endpoint_path = `${outdir}/endpoints/${safe_path.slice(1) ? safe_path.slice(1) + '/route.js': 'route.js'}`
